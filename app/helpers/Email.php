<?php

namespace app\helpers;

use Exception;
use PHPMailer\PHPMailer\PHPMailer;

class Email 
{
    private string $from;
    private string|array $to;
    private string $template = '';
    private array $templateData = [];
    private string $nameFrom;
    private string|array $nameTo;
    private string $subject;
    private string $message;
    private PHPMailer $mail;

    public function __construct()
    {
        $this->mail = new PHPMailer(true);
        $this->mail->isSMTP();
        $this->mail->Host = env('EMAIL_HOST');
        $this->mail->SMTPAuth = true;
        $this->mail->Username = env('EMAIL_USERNAME');
        $this->mail->Password = env('EMAIL_PASSWORD');
        $this->mail->Port = env('EMAIL_PORT');
    }

    public function from(string $from, string $name = ''): Email
    {
        $this->from = $from;
        $this->nameFrom = $name;
        return $this;
    }

    public function to(string|array $to, string|array $name = ''): Email 
    {
        $this->to = $to;
        $this->nameTo = $name;
        return $this;
    }

    public function template(string $template, array $templateData): Email 
    {
        $this->template = $template;
        $this->templateData = $templateData;

        return $this;
    }

    public function message(string $msg): Email 
    {
        $this->message = $msg;
        return $this;
    }

    public function subject(string $subject): Email
    {
        $this->subject = $subject;
        return $this;
    }

    private function sendWithTemplate() 
    {
        $file = '../app/views/emails/'. $this->template . '.html';

        if(!file_exists($file)) {
            throw new Exception("O template do email {$this->template} não existe");
        }

        $template = file_get_contents($file);

        $this->templateData['message'] = $this->message;

        foreach($this->templateData as $key => $data) {
            $dataTemplate["@{$key}"] = $data;
        }

        return \str_replace(array_keys($dataTemplate), array_values($dataTemplate), $template);
    }

    public function send()
    {
        $this->mail->setFrom($this->from, $this->nameFrom);
        
        if(\is_string($this->to) && \is_string($this->nameTo)) {
            $this->mail->addAddress($this->to, $this->nameTo);
        }

        if(is_array($this->to) && is_array($this->nameTo)) {
            foreach($this->to as $index => $to) {
                $this->mail->addAddress($to, $this->nameTo[$index]); 
            }
        } 

        $this->mail->isHTML(true);   
        $this->mail->CharSet = 'utf-8';
        $this->mail->Subject = $this->subject;
        $this->mail->Body    = empty($this->template) ? $this->message : $this->sendWithTemplate();
        $this->mail->AltBody = $this->message;

        return $this->mail->send();
    }
}